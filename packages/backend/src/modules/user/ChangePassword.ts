...
export class ChangePasswordResolver {
	@Mutation(() => User)
	async changePassword(@Arg('data') {token, password}: ChangePasswordInput, @Ctx() ctx: Context): Promise<User> {
		const userId = await redis.get(UserConfirmationPrefixes.forgotPassword + token);
